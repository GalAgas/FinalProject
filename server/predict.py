...
def predict_MIC(models_dir, spceis, csv_file, txt_file):
    models_dir = 'models'
    ...
    for anti_dir in tqdm(os.listdir(models_dir + '/' + spceis)):
        if '.ipynb' in anti_dir:
            continue
        # problem!!!!!!
        model, data_param, model_param, features = get_model(
            models_dir + '/' + spceis + '/' + anti_dir)
        # X = parse_sample(sample_path_dir, data_param, features)
        print("step 2")
        print("step 3")
        X = parse_sample(csv_file, txt_file, data_param, features)
...
def parse_sample(csv_file, txt_file, data_param, features):
    print(data_param)
    X = get_isolate_features(csv_file, txt_file)
...
def model_predict(X, model, model_param):
    ...
    if model_param['model_name'] == 'h2o':
        ...
        X['biosample_id'] = 0
        X.to_csv(sample_path)
        PredictAutoXGBCommand(model, sample_path,
                              '{}/preds.csv'.format(data_dir)).execute()
