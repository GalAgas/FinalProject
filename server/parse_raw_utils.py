...
def save_filtered_metadata(tot_data, all_ASR, ds_name):
    ...
    filtered_ASR = all_ASR[all_ASR['biosample_id'].isin(
        tot_data['biosample_id'])]
...
def get_metadata(resources_path):
    ...
    for file_format, resources_dict in resources_path.items():
        if file_format in ['VAMP', 'PATAKI']:
            for sam_dir in tqdm(os.listdir(resources_dict['pheno'])):
                sam_phen = load_all_phen_data(
                    resources_dict['pheno']+'/'+sam_dir, file_format=file_format)
            ...
            sam_phen = load_all_phen_data(
                resources_dict['pheno'], resources_dict['run2bio'], file_format=file_format)
            all_ASR = pd.concat([all_ASR, sam_phen], axis=0)
    if len(error_id) == 0:
        error_id = None
    ...
    all_ASR['measurement_type'].fillna(
        all_ASR['laboratory_typing_method'], inplace=True)
    ...
    all_ASR.drop(['platform ', 'bioSample_ID', 'measurement_units',
                 'laboratory_typing_method', ], axis=1, inplace=True)
...
def load_all_phen_data(
    ...
    try:
        if file_format == 'PATAKI':
            ...
            run2biosam = run2biosam[['PATRIC_ID',
                                     'sample_accession', 'Species']]
            run2biosam.columns = ['genome_id', 'biosample_id', 'species_fam']
            run2biosam['genome_id'] = run2biosam['genome_id'].astype(str)
            phen_df = phen_df.merge(right=run2biosam, how='inner', on='genome_id').drop(
                ['genome_id'], axis=1)
            phen_df.rename(
                columns={'antibiotic': 'antibiotic_name'}, inplace=True)
    except:
    ...
    phen_df['measurement_sign'].replace(
        inplace=True, to_replace='==', value='=')

    phen_df['antibiotic_name'] = phen_df['antibiotic_name'].str.lower()
    phen_df['antibiotic_name'] = phen_df['antibiotic_name'].replace(
        ' ', '_', regex=True)
    phen_df['antibiotic_name'] = phen_df['antibiotic_name'].replace(
        '-', '_', regex=True)

    if file_format == 'PATRIC':
        phen_df['measurement'] = phen_df.apply(
            lambda row: fix_PATRIC_MIC_value(row, '1'), axis=1)
        phen_df['measurement2'] = phen_df.apply(
            lambda row: fix_PATRIC_MIC_value(row, '2'), axis=1)
        phen_df['measurement_has_/'] = phen_df.apply(
            lambda row: fix_PATRIC_MIC_value(row, 'has'), axis=1)
...
def merge_gen2phen(geno, pheno, run2bio_match_path, filter_list_path, db_name):
    ...
    if db_name == 'PATRIC':
        run2biosam['PATRIC_ID'] = run2biosam['PATRIC_ID'].astype(str)
        run2biosam = run2biosam[['run_accession',
                                 'PATRIC_ID', 'sample_accession', 'Species']]
        run2biosam.columns = ['run_id', 'genome_id',
                              'biosample_id', 'species_fam']
        run2biosam['genome_id'] = run2biosam['genome_id'].astype(str)
    else:
    ...
    if db_name == 'PATRIC':
        filtered_data = run2biosam.merge(
            right=pheno, how='inner', on='genome_id')
        filtered_data = filtered_data.drop(['genome_id'], axis=1)
    else:
        filtered_data = filtered_data.merge(
            right=run2biosam, how='inner', on='run_id')
        filtered_data = filtered_data.merge(
            right=pheno, how='inner', on='biosample_id')
    filtered_data = filtered_data.merge(right=geno, how='inner', on='run_id')
    filtered_data['DB'] = db_name
    filtered_data = filtered_data.drop_duplicates(
        subset='biosample_id', keep='first')
...
def get_phenotype_per_db(AST_dir, db_name='PATAKI'):
    ...
    if db_name == 'VAMP' or db_name == 'PATAKI':
        ...
        for sam_dir in tqdm(os.listdir(AST_dir)):
            sam_labels = get_isolate_labels(
                AST_dir+'/'+sam_dir, file_format=db_name)
        ...
        phenotypic['antibiotic'] = phenotypic['antibiotic'].replace(
            ' ', '_', regex=True)
        phenotypic['antibiotic'] = phenotypic['antibiotic'].replace(
            '-', '_', regex=True)
        ...
        phenotypic['measurement'] = phenotypic.apply(
            fix_PATRIC_MIC_value, axis=1)
        phenotypic.drop_duplicates(
            subset=['genome_id', 'antibiotic'], keep='first', inplace=True)
        phenotypic = phenotypic.pivot(
            index='genome_id', values='measurement', columns='antibiotic')
        error_id = None
    else:
...
def get_isolate_features(
    ...
    gene_df = gene_df.groupby(by='Gene').apply(
        lambda x: x.iloc[x[sortby].argmax()])
    gene_df = gene_df[['Contig', 'Start', 'End', 'Depth', 'SeqID',
                       'SeqCov', 'Match_Start', 'Match_End', 'Ref_Gene_Size']].reset_index()
    ...
    contig_df['contig_size'] = contig_df['contig_size'].apply(
        lambda x: int(x[4:]))
    contig_df['avg_depth'] = contig_df['avg_depth'].apply(
        lambda x: float(x[4:]))

    features = gene_df.merge(right=contig_df, how='left', left_on='Contig',
                             right_on='contig_id').drop(['Contig', 'contig_id'], axis=1)
    features['dist_contig_end'] = np.minimum(
        features['Start'], features['contig_size']-features['End'])
    features['contig_end_partition'] = features['dist_contig_end'] / \
        features['contig_size']
    features['relative_depth'] = features['Depth'] / features['avg_depth']
    features = features[['Gene', 'Depth', 'avg_depth', 'relative_depth',
                         'dist_contig_end', 'contig_end_partition', 'SeqID', 'SeqCov']]
    features.columns = ['gene', 'depth', 'avg_depth', 'relative_depth',
                        'dist_contig_end', 'contig_end_partition', 'seq_id', 'seq_cov']
    if not with_confidence:
        features = features.drop(
            ['relative_depth', 'depth', 'avg_depth', 'dist_contig_end', 'contig_end_partition'], axis=1)
    features = features.set_index('gene').stack().reset_index()
    features['col_name'] = features['gene'].values + \
        '->' + features['level_1'].values
...
def get_isolate_labels(
    ...
    phen_df['antibiotic_name'] = phen_df['antibiotic_name'].replace(
        '-', '_', regex=True)
    if MIC_val:
        phen_df = phen_df[['antibiotic_name',
    ...
    phen_df['label'] = phen_df['measurement_sign'].values + \
        ' ' + phen_df['label'].astype(str).values
    phen_df = phen_df.sort_values('measurement', ascending=False).drop_duplicates(
        subset=['antibiotic_name'], keep='first')

    labels = phen_df[['label', 'antibiotic_name']
                     ].set_index('antibiotic_name').T
...
def main():
    ...
    tot_data, labels, features = merging_dbs(
        filtered_data_list, labels_list, features_list)
